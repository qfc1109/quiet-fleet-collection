...
## 项目级 RAG 知识库设计（规划，尚未实现）

本节与 [ADR-001](adr/2026-09-05-project-rag-knowledge-base.md) 保持同步。当前代码没有以下表、接口、索引或模型调用。

### 边界与权限

- `project.id` 是逻辑知识库边界，不预设外部平台的知识库 ID。
- 新增项目级成员关系，角色为 `VIEWER` / `EDITOR`；两者都可查询负责人纳入问答的文件，`VIEWER` 只读问答，`EDITOR` 可编辑项目文件；成员管理和外部共享仅负责人可操作，暂不做文件级 ACL。
- 负责人控制成员、纳入文件、外部数据共享和失败重试。管理员创建且没有负责人的项目，仅后台管理员可以指定网站用户负责人；指定负责人前不能启用问答。
- 服务端必须先解析网站用户 session、项目和成员关系，再执行检索；不能复用只按 `fileId` 读取的公开文件路径作为权限依据。

### 资料和索引

- 文件默认不入库，由负责人显式选择文件或目录；目录选择只形成当前文件快照，新增文件必须再次显式选择；后端维护敏感文件 denylist，并阻止密钥、数据库、压缩包等高风险资料。
- 最小概念模型包括 `project_member`、知识文件选择/状态、文本分块和引用定位。具体表名、字段和 Embedding 存储方式在实现设计阶段确定。
- 状态至少包括“待处理、索引中、可用、失败”。上传/修改走可重试的异步增量流程；删除或撤权先从召回过滤，再清理索引。
- MVP 首批只抽取 Markdown、纯文本和 Word；PDF 后置。现有 Markdown/Word/Excel 预览逻辑不能直接视为通用抽取器；编码检测、大小/字符上限和复杂文档解析必须单独定义。

### 外部模型与回答

- 只有负责人显式选中的非敏感资料可发送到外部模型服务，页面显示数据去向，密钥仅保存在后端配置。
- 通过可配置的 OpenAI 兼容 REST 自定义模型适配器接入模型；模型名固定为 `gpt-5.6-terra`，`baseUrl`、API Key、超时和额度均配置化，实际服务地址和凭据在部署时提供，不绑定某个供应商 SDK。API Key 只放被忽略的本地/部署配置，不能进入仓库。
- MVP 检索先使用项目内简单文本/关键词召回；是否引入 Embedding/托管知识库以后再由监控和反馈决定。
- 回答只允许引用检索资料；无足够依据时返回“资料中没有找到”。文档片段视为不可信输入，不得触发工具调用、越权读取或系统提示覆盖。
- 第一版使用普通 JSON；响应至少包含回答和 `citations[{fileId, relativePath, quote, locator}]`。引用无法精确定位时退化为打开原文件。
- 页面内只保留最近三组问答，不保存聊天正文；回答返回临时 `answerId`，提供“准确 / 不准确”反馈，每个用户对同一回答只保留一次评价，反馈只保存 `answerId`、`userId`、`vote`、`projectId`、`createdAt` 等元数据。

### 规模与非目标

- MVP 约束为不超过 100 个项目、每项目 500 个可索引文件、同时 5 个问答；规模超过后再依据监控迁移检索设施。
- 不在本阶段实现匿名问答、文件级 ACL、持久化聊天、流式输出、图片 OCR、复杂版式解析或外部来源同步。
