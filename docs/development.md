...
## 项目级 RAG 开发约束（阶段一 MVP 已实现）

详细决策见 [ADR-001：项目级 RAG 知识库与文档问答](adr/2026-09-05-project-rag-knowledge-base.md) 和 [术语表](glossary.md)。以下内容是后续开发的约束，不表示当前接口已经存在。

### 现有能力限制

- `server/pom.xml` 当前没有 LLM/Embedding SDK、向量库、全文检索或消息队列；模型通过可配置的 OpenAI 兼容 REST 自定义适配器接入，模型名固定为 `gpt-5.6-terra`，`baseUrl`、API Key、超时和额度均配置化。若协议允许，优先用现有 Spring Web 的 `RestTemplate`，并自行配置错误映射和重试上限；实际服务地址和凭据在部署时提供。API Key 只能通过被忽略的本地/部署配置注入，禁止写入源码、示例配置或日志。
- Redis 当前用于 Session 和 token，不把可恢复的索引任务或聊天历史默认为 Session 数据；索引需要可重试的 worker/outbox，不能把外部模型调用绑定在上传 HTTP 请求事务内。
- 文件正文仍在本地 `storage`，`project_file` 只有元数据。新增索引数据需要独立迁移，并纳入备份、磁盘和回滚说明；当前项目没有 Flyway/Liquibase，生产迁移仍需手工执行。
- MVP 只抽取 Markdown、纯文本和 Word；PDF 后置。现有 Markdown/Word/Excel 预览代码不能直接充当生产抽取器；必须增加格式白名单、编码/大小/字符限制、分块和定位信息。目录选择只形成当前文件快照，新增文件必须再次显式选择；图片、密钥、数据库、压缩包和其他高风险文件不可进入索引。

### 接口和前端约束

- 规划中的聊天入口挂在项目上下文中，优先使用 `/api/space/projects/{projectId}/chat`；不新增匿名公开问答入口。`VIEWER` 只读问答，`EDITOR` 可编辑项目文件，成员管理和外部共享仅负责人可操作。
- 聊天响应采用普通 JSON，前端单独设置约 30 秒超时；当前 Nginx API 超时约 60 秒，第一版不做 SSE/WebSocket。
- 引用必须能回到项目文件，服务端重新校验项目、成员、文件启用状态和删除状态；不能只信任前端传入的 `fileId` 或索引元数据。
- 页面显示文件索引状态和知识库不完整提示；每个回答返回临时 `answerId` 并提供“准确 / 不准确”反馈，每个用户对同一回答只保留一次评价，反馈只保存 `answerId`、`userId`、`vote`、`projectId`、`createdAt` 等元数据，不保存聊天正文。

### 上线前置检查

- 先修复公共文件按 `fileId` 读取时缺少项目可见性/删除状态复核的问题，再复用文件引用链路。
- 盘点并处理公开项目中的疑似密钥、证书、配置和脚本文件；若确认是真实凭据，先撤下并轮换，禁止送往外部模型。
- 在配置文件或部署环境注入模型 endpoint、密钥、模型和额度限制，不提交仓库；确认服务器出网策略和外部数据合规边界。
- 先完成成员关系、负责人指定、文件显式选择、简单文本/关键词召回、索引状态和手工迁移设计，再开始代码实现；负责人指定操作仅后台管理员可执行。
